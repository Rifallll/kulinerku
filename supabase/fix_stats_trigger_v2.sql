-- ==========================================
-- FINAL STATS TRIGGER FIX (COLUMN SYNC) 📊
-- ==========================================
...
-- 3. VIEWS SYNC (UPDATED: activity_type & 'view')
create or replace function public.handle_view_change()
returns trigger as $$
begin
  -- Check for correct column and value ('view')
  if (TG_OP = 'INSERT' and new.activity_type = 'view') then
...
-- 4. FINAL RECALCULATION (Updated column name)
...
set 
  ...
  -- Fix: Check activity_type = 'view'
  total_foods_viewed = (select count(*) from public.user_activities a where a.user_id = s.user_id and a.activity_type = 'view');

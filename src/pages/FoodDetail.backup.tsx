...
const FoodDetail = () => {
  ...
  const { data: food, isLoading, isError, error } = useQuery<FoodItem, Error>({
  ...
  const { data: relatedFoods } = useQuery<FoodItem[]>({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase.from("food_items").select("*").or(`type.eq.${food.type},origin.ilike.%${province}%`).neq('id', food.id).limit(6);
      if (!data) return [];
